...
#ifndef WEBSOCKETPP_TRANSPORT_BARE_HPP
#define WEBSOCKETPP_TRANSPORT_BARE_HPP

#include <websocketpp/transport/base/endpoint.hpp>
#include <websocketpp/transport/bare/connection.hpp>
#include <websocketpp/transport/bare/security/none.hpp>

#include <websocketpp/uri.hpp>
#include <websocketpp/logger/levels.hpp>

#include <websocketpp/common/bare.hpp>
...
public:
    ...
    void init_bare(io_service_ptr ptr, lib::error_code & ec) {
        ...
        m_alog->write(log::alevel::devel,"bare::init_bare");

        m_io_service = ptr;
        m_external_io_service = true;
        m_acceptor.reset(new lib::bare::ip::tcp::acceptor(*m_io_service));
...
#else
        lib::auto_ptr<lib::bare::io_service> service(new lib::bare::io_service());
#endif
        init_bare(service.get(), ec);
     ...
     *
     * The default value is specified as *::bare::socket_base::max_connections
     ...
     * `init_bare`. No error will be returned if it isn't.
     *
    ...
    lib::bare::ip::tcp::endpoint get_local_endpoint(lib::bare::error_code & ec) {
        ...
        if (m_acceptor) {
            ...
            ec = lib::bare::error::make_error_code(lib::bare::error::bad_descriptor);
    ...
    void listen(lib::bare::ip::tcp::endpoint const & ep, lib::error_code & ec)
    {
        ...
        m_alog->write(log::alevel::devel,"bare::listen");

        lib::bare::error_code bec;

        m_acceptor->open(ep.protocol(),bec);
        if (bec) {ec = clean_up_listen_after_error(bec);return;}
        
        m_acceptor->set_option(lib::bare::socket_base::reuse_address(m_reuse_addr),bec);
        if (bec) {ec = clean_up_listen_after_error(bec);return;}
     ...
     * - IPv6 with mapped IPv4 for dual stack hosts lib::bare::ip::tcp::v6()
     * - IPv4 only: lib::bare::ip::tcp::v4()
     *
        ...
        lib::error_code & ec)
    {
        ...
        if (endpoint_iterator == end) {
            m_elog->write(log::elevel::library,
                "bare::listen could not resolve the supplied host or service");
        ...
        if (ec) {
            if (ec == lib::bare::error::operation_aborted) {
                ...
                m_elog->write(log::elevel::info,
                    "bare handle_timer error: "+ec.message());
                log_err(log::elevel::info,"bare handle_timer",ec);
        ...
        lib::error_code & ec)
    {
        ...
        m_alog->write(log::alevel::devel, "bare::async_accept");
...
protected:
        ...
        bare_ec)
    {
        ...
        if (bare_ec) {
            if (bare_ec == lib::bare::errc::operation_canceled) {
                ret_ec = make_error_code(websocketpp::error::operation_canceled);
            } else {
                log_err(log::elevel::info,"bare handle_accept",bare_ec);
                ret_ec = socket_con_type::translate_ec(bare_ec);
            }
        }
        ...
        lib::error_code const & ec)
    {
        ...
        if (ec) {
            if (ec == transport::error::operation_aborted) {
                m_alog->write(log::alevel::devel,
                    "bare handle_resolve_timeout timer cancelled");
                return;
            }

            log_err(log::elevel::devel,"bare handle_resolve_timeout",ec);
            ret_ec = ec;
        } else {
...
/*
    ...
    {
        if (ec == lib::bare::error::operation_aborted ||
            lib::bare::is_neg(dns_timer->expires_from_now()))
        {
        ...
        if (ec) {
            log_err(log::elevel::info,"bare async_resolve",ec);
            callback(socket_con_type::translate_ec(ec));
            return;
        }
        ...
        if (m_alog->static_test(log::alevel::devel)) {
            ...
            lib::bare::ip::tcp::resolver::iterator it, end;
...
*/
        ...
        connect_handler callback, lib::error_code const & ec)
    {
        ...
        if (ec) {
            if (ec == transport::error::operation_aborted) {
                m_alog->write(log::alevel::devel,
                    "bare handle_connect_timeout timer cancelled");
                return;
            }

            log_err(log::elevel::devel,"bare handle_connect_timeout",ec);
            ret_ec = ec;
        } else {
            ret_ec = make_error_code(transport::error::timeout);
        }
        ...
        connect_handler callback, lib::bare::error_code const & ec)
    {
        ...
        if (ec) {
            log_err(log::elevel::info,"bare async_connect",ec);
            callback(socket_con_type::translate_ec(ec));
            return;
        }
    ...
    lib::error_code init(transport_con_ptr tcon) {
        ...
        ec = tcon->init_bare(m_io_service);
        if (ec) {return ec;}
